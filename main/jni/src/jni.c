...
static void handle_signal(int sig, siginfo_t *info, void *ctx)
{
    ...
    jmethodID get_logger = (*env)->GetStaticMethodID(env, org_slf4j_logger_factory, "getLogger", "(Ljava/lang/String;)Lorg/slf4j/Logger;");
    jobject logger = (*env)->CallStaticObjectMethod(env, org_slf4j_logger_factory, get_logger, (*env)->NewStringUTF(env, TAG));
    // logger.error(msg);
